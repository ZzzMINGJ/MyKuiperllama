...
void emb_kernel_cu(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  if (stream) {
    ...
    emb_kernel_cu_fp32<<<input_num, thread_num, 0, stream_>>>(vocab_size, input_num, weight_dim,
                                                               in_ptr, wei_ptr, out_ptr);
  } else {
    emb_kernel_cu_fp32<<<input_num, thread_num>>>(vocab_size, input_num, weight_dim, in_ptr,
                                                  wei_ptr, out_ptr);
  }
}
}  // namespace kernel